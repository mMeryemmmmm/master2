...
# Data
Tests = []
data='C:\\Users\\SAMSUNG\\Desktop\\Mermiem-master\\dataaprotine-1.csv '
datapred='C:\\Users\\SAMSUNG\\Desktop\\Mermiem-master\\predict.csv '
app = Flask(__name__)
cors = CORS(app)
...
def Neighbors():
    ...
    print('Accuracy pour K-NN classifier on training set: {:.2f}'.format(knn.score(X_train, y_train)))
...
def Arbrepredict():
    title = request.get_json()
    print(title['Name'])
    myCsvRow = str(title['Name']) + ',' + 'LSN2322600|LY2228820 DIMESYLATE|RALIMETINIB MESYLATE' + ',' + str(
        title['Minimum']) + ',' + '612.75,2,4' + ',' + str(title['Number']) + ',' + ',' + str(
        title['measured']) + ',' + 'CC12CCC3C(CCc4cc(O)ccc34)C2CCC1O,1 1 1 0 0 0 0 1 1 0,0'
    with open('predict.csv','a') as fd:
      fd.write("\n"+myCsvRow)
    Tests = []
    moleculs = pd.read_csv(data)
